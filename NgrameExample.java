...
public class BigramPMI {
	...
	public static class MapClass extends
		...
		public void map(LongWritable key, Text value, Context context)
			...
			if (ngram_words.length != 5) {
				return;
			}

			context.write(new Text(ngram_words[2]),
	...
	public static class PartitionerClass extends Partitioner<Text, IntWritable> {
		// Partitioner implementation: divide the keys according to their
		// language: Hebrew - 1, Other - 0
		...
		private int getLanguage(Text key) {
			if (key.getLength() > 0) {
				...
				if (c >= Long.decode("0x05D0").longValue()
						&& c <= Long.decode("0x05EA").longValue()) // Hebrew
																	// character
					return 1;
			}
			return 0;
	...
	public static void main(String[] args) throws Exception {
		...
		job.setJarByClass(BigramPMI.class);
